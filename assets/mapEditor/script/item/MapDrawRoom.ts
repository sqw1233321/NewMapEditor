...
import { MapDrawDatDoor, MapDrawDatEnemyRefreshData, MapDrawDatLadder, MapDrawDatRoom, MapDrawDatSearchItemData } from "./MapDrawDat";
...
export default class MapDrawRoom extends MapDrawUnitBase {
    ...
    public setDat() {
        ...
        this._pointIds = this._points?.map((point: MapDrawP) => point.getId()) || [];
        this._unLockPointIds = this.unLockPoints.map((point: cc.Node) => point.getComponent(MapDrawP).getId());
        this.setDoorDat();
        this.setLadderDat();
        this.setEnemyDat();
